...
/**
 * <b>CompatibleMaterial</b> - Data Values/Pre-flattening<br>
 ...
 * CompatibleMaterial v1: https://www.spigotmc.org/threads/329630/
 * <p>
...
public enum CompatibleMaterial {
    ...
    /**
     * Cached array of {@link CompatibleMaterial#values()} to avoid allocating memory for
     * calling the method every time.
     *
     * @since 2.0.0
     */
    public static final CompatibleMaterial[] VALUES = values();
     ...
     * @since 5.1.0
     */
    private static final Map<String, CompatibleMaterial> NAMES = new HashMap<>();

    /**
     * Guava (Google Core Libraries for Java)'s cache for performance and timed caches.
     * For strings that match a certain CompatibleMaterial. Mostly cached for configs.
     *
     * @since 1.0.0
     */
    private static final Cache<String, CompatibleMaterial> NAME_CACHE = CacheBuilder.newBuilder()
     ...
     *
     * @see #matchCompatibleMaterialWithData(String)
     * @since 8.0.0
     */
    ...
    /**
     * <b>CompatibleMaterial Paradox (Duplication Check)</b>
     * <p>
    ...
    static {
        for (CompatibleMaterial material : VALUES) NAMES.put(material.name(), material);
    }
    ...
    CompatibleMaterial(int data, int version, @Nonnull String... legacy) {
        this.data = (byte) data;
    ...
    CompatibleMaterial(int data, @Nonnull String... legacy) {
        this(data, 0, legacy);
    }

    CompatibleMaterial(int version) {
        this(0, version);
    }
    ...
    /**
     * Gets the CompatibleMaterial with this name similar to {@link #valueOf(String)}
     ...
     */
    @Nonnull
    private static Optional<CompatibleMaterial> getIfPresent(@Nonnull String name) {
     ...
     *
     * @see #matchDefinedCompatibleMaterial(String, byte)
     * @since 1.0.0
     */
    @Nullable
    private static CompatibleMaterial requestOldCompatibleMaterial(@Nonnull String name, byte data) {
        String holder = name + data;
        CompatibleMaterial cache = NAME_CACHE.getIfPresent(holder);
        if (cache != null) return cache;

        for (CompatibleMaterial material : VALUES) {
    ...
    /**
     * Parses the given material name as an CompatibleMaterial with a given data
     * value in the string if attached. Check {@link #matchCompatibleMaterialWithData(String)} for more info.
     *
     * @see #matchCompatibleMaterialWithData(String)
     * @see #matchDefinedCompatibleMaterial(String, byte)
     * @since 2.0.0
     */
    @Nonnull
    public static Optional<CompatibleMaterial> matchCompatibleMaterial(@Nonnull String name) {
        Validate.notEmpty(name, "Cannot match a material with null or empty material name");
        Optional<CompatibleMaterial> oldMatch = matchCompatibleMaterialWithData(name);
        return oldMatch.isPresent() ? oldMatch : matchDefinedCompatibleMaterial(format(name), UNKNOWN_DATA_VALUE);
    }
     ...
     * @return the parsed CompatibleMaterial.
     * @see #matchCompatibleMaterial(String)
     * @since 3.0.0
     */
    @Nonnull
    private static Optional<CompatibleMaterial> matchCompatibleMaterialWithData(@Nonnull String name) {
        ...
        if (index != -1) {
            ...
            try {
                ...
                return data >= 0 && data < MAX_DATA_VALUE ? matchDefinedCompatibleMaterial(mat, data) : matchDefinedCompatibleMaterial(mat, UNKNOWN_DATA_VALUE);
            } catch (NumberFormatException ignored) {
                return matchDefinedCompatibleMaterial(mat, UNKNOWN_DATA_VALUE);
            }
        }
    ...
    /**
     * Parses the given material as an CompatibleMaterial.
     *
     * @throws IllegalArgumentException may be thrown as an unexpected exception.
     * @see #matchDefinedCompatibleMaterial(String, byte)
     * @see #matchCompatibleMaterial(ItemStack)
     * @since 2.0.0
     */
    @Nonnull
    public static CompatibleMaterial matchCompatibleMaterial(@Nonnull Material material) {
        Objects.requireNonNull(material, "Cannot match null material");
        return matchDefinedCompatibleMaterial(material.name(), UNKNOWN_DATA_VALUE)
                .orElseThrow(() -> new IllegalArgumentException("Unsupported material with no data value: " + material.name()));
    }

    /**
     * Parses the given item as an CompatibleMaterial using its material and data value (durability)
     * if not a damageable item {@link ItemStack#getDurability()}.
     *
     * @param item the ItemStack to match.
     * @return an CompatibleMaterial if matched any.
     * @throws IllegalArgumentException may be thrown as an unexpected exception.
     * @see #matchCompatibleMaterial(Material)
     * @since 2.0.0
     */
    @Nonnull
    @SuppressWarnings("deprecation")
    public static CompatibleMaterial matchCompatibleMaterial(@Nonnull ItemStack item) {
        ...
        return matchDefinedCompatibleMaterial(material, data)
                .orElseThrow(() -> new IllegalArgumentException("Unsupported material from item: " + material + " (" + data + ')'));
    }

    /**
     * The main method that parses the given material name and data value as an CompatibleMaterial.
     ...
     * @return an CompatibleMaterial (with the same data value if specified)
     * @see #matchCompatibleMaterial(Material)
     * @see #matchCompatibleMaterial(int, byte)
     * @see #matchCompatibleMaterial(ItemStack)
     * @since 3.0.0
     */
    @Nonnull
    protected static Optional<CompatibleMaterial> matchDefinedCompatibleMaterial(@Nonnull String name, byte data) {
        ...
        if (Data.ISFLAT || (data <= 0 && !(duplicated = isDuplicated(name)))) {
            Optional<CompatibleMaterial> CompatibleMaterial = getIfPresent(name);
            if (CompatibleMaterial.isPresent()) return CompatibleMaterial;
        }
        // Usually flat versions wouldn't pass this point, but some special materials do.

        CompatibleMaterial oldCompatibleMaterial = requestOldCompatibleMaterial(name, data);
        if (oldCompatibleMaterial == null) {
            // Special case. Refer to FILLED_MAP for more info.
            return data > 0 && name.endsWith("MAP") ? Optional.of(FILLED_MAP) : Optional.empty();
        }

        if (!Data.ISFLAT && oldCompatibleMaterial.isPlural() && (duplicated == null ? isDuplicated(name) : duplicated)) return getIfPresent(name);
        return Optional.of(oldCompatibleMaterial);
    }
    ...
    private static boolean isDuplicated(@Nonnull String name) {
        // Don't use matchCompatibleMaterial() since this method is being called from matchCompatibleMaterial() itself and will cause a StackOverflowError.
        return DUPLICATED.contains(name);
    }

    /**
     * Gets the CompatibleMaterial based on the material's ID (Magic Value) and data value.<br>
     ...
     * @return a parsed CompatibleMaterial with the same ID and data value.
     * @see #matchCompatibleMaterial(ItemStack)
     * @since 2.0.0
    ...
    @Nonnull
    @Deprecated
    public static Optional<CompatibleMaterial> matchCompatibleMaterial(int id, byte data) {
        if (id < 0 || id > MAX_ID || data < 0) return Optional.empty();
        for (CompatibleMaterial materials : VALUES) {
    ...
    /**
     * CompatibleMaterial Paradox (Duplication Check)
     ...
     * <p>
     * A solution for CompatibleMaterial Paradox.
     ...
     * <b>Example:</b>
     * <blockquote><pre>
     *     CompatibleMaterial material = {@link #matchCompatibleMaterial(ItemStack)};
    ...
    public boolean isOneOf(@Nullable Collection<String> materials) {
        ...
        for (String comp : materials) {
            ...
            Optional<CompatibleMaterial> xMat = matchCompatibleMaterial(comp);
    ...
    /**
     * Checks if the given material name matches any of this CompatibleMaterial's legacy material names.
     * All the values passed to this method will not be null or empty and are formatted correctly.
     *
    ...
    /**
     * Parses an item from this CompatibleMaterial.
    ...
    /**
     * Parses the material of this CompatibleMaterial.
     *
     * @return the material related to this CompatibleMaterial based on the server version.
     * @since 1.0.0
     */
    @Nullable
    ...
    private boolean isDuplicated() {
        switch (this.name()) {
            ...
            case "NETHER_BRICK":
                ...
                // Illegal Elements
                // Since both 1.12 and 1.13 have <type>_DOOR CompatibleMaterial will use it
                // for 1.12 to parse the material, but it needs <type>_DOOR_ITEM.
                // We'll trick CompatibleMaterial into thinking this needs to be parsed
